...
export default function Notes() {
  ...
  // Load today's note into form if exists
  useEffect(() => {
    ...
    if (todayNote) {
      setForm({
        ...
        mood: todayNote.mood || 'okay',
        tags: todayNote.tags?.join(', ') || ''
      });
    }
  }, [notes]);
  ...
  const filteredNotes = notes.filter(n =>
    !filter || n.tags?.some(t => t.toLowerCase().includes(filter.toLowerCase())) ||
    ...
    <div className="notes-page">
      <div className="card">
        ...
        <form onSubmit={handleSave} className="note-form">
          ...
          <textarea
            placeholder="What happened today? What did you accomplish? How do you feel?..."
            value={form.content}
          ...
          />
          <input
            placeholder="Tags: productive, tired, fun (comma separated)"
            value={form.tags}
