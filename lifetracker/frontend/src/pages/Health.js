    ...
    <div className="health-page">
      <div className="card">
        ...
        <form onSubmit={handleSave} className="health-form">
          ...
          <input placeholder="Notes (e.g. after workout)" value={form.notes}
      ...
      <div className="card">
        ...
        <div className="health-history">
          {entries.map((entry, i) => {
            ...
            const diff = prev?.weight && entry.weight ? (entry.weight - prev.weight).toFixed(1) : null;
            return (
              <div key={entry._id} className="health-row">
                ...
                {diff && (
                  <span className={`health-diff ${diff > 0 ? 'up' : 'down'}`}>
                    {diff > 0 ? `+${diff}` : diff}
                  </span>
                )}
